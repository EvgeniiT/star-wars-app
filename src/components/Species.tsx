import { Autocomplete, TextField } from "@mui/material";
import { useLoaderData } from "@tanstack/react-router";
import { memo, useCallback, useEffect, useState } from "react";
import { useSpecies } from "../api/useSpecies";
import { Option } from "../types";
import { FormData } from "./EditCharacter";

type Props = {
  value: Option[];
  setFormData: React.Dispatch<React.SetStateAction<FormData>>;
};
function Species({ value, setFormData }: Props) {
  const { speciesPromise } = useLoaderData({
    from: "/$id",
  });

  const [search, setSearch] = useState("");

  const setValue = useCallback(
    (newValue: Option[]) => {
      setFormData((prev) => ({
        ...prev,
        species: newValue,
      }));
    },
    [setFormData]
  );
  const { data, isPending } = useSpecies({
    page: 1,
    search,
  });
  const options =
    data?.results.map((d) => ({
      id: d.url,
      label: d.name,
    })) || [];

  useEffect(() => {
    const setField = async () => {
      const species = await speciesPromise;
      const options = species.map((species) => ({
        id: species.url,
        label: species.name,
      }));
      setValue(options);
    };
    setField();
  }, [speciesPromise, setValue]);

  return (
    <Autocomplete
      multiple
      options={options || []}
      onChange={(e: React.SyntheticEvent<Element, Event>, newValue: Option[]) =>
        setValue(newValue)
      }
      value={value}
      onInputChange={(_, v) => setSearch(v)}
      inputValue={search}
      renderInput={(params) => <TextField {...params} label="Species" />}
      getOptionLabel={(option) => option.label}
      loading={isPending}
    />
  );
}

export default memo(Species);
